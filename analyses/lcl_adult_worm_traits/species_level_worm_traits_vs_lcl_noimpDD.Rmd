...
```
```{r}
dat <- mutate(dat, log_end = log(biovolume), log_start = log(initial_biov),
              log_dt = log(avg_dd),
...
dat <- mutate(dat, avg_dd_p = if_else(is_paratenic == 1, 3, avg_dd))
...
incomp_spp <- dat%>%
  filter(is.na(avg_dd_p), is_paratenic != 1, Facultative != 'postcyclic')%>%
  .$Parasite.species
...
```{r}
lcdb_lhl <- select(dx, Parasite.species, lcl_max_fac, Host_no_fac, cum_dt, propagule, biovolume)%>%
  pivot_longer(cols = propagule:biovolume)
lcdb_lhl <- mutate(lcdb_lhl, cum_dt = if_else(name == 'propagule', 1, cum_dt))
...
We fit the same three models described above: intercept-only, life cycle length as continuous variable, and life cycle length as categorical variable. In all, taxonomy is included as nested random effects.
...
```
```{r}
cat("DIC, life cycle length (cont);", chains1$DIC )
```
```{r}
cat("DIC, life cycle length (cat);", chains2$DIC )
```
...
```{r}
cat('Delta DIC, life cycle length (cont) vs lcl (categorical):', 
...
```{r}
lcl_effect <- exp(chains1$Sol[,'traitlog_end:lcl_max'])-1
summary(lcl_effect)
```
...
```{r}
lcl_effect <- exp(chains1$Sol[,'traitlog_propagule:lcl_max'])-1
summary(lcl_effect)
```
...
```{r}
lcl_effect <- exp(chains1$Sol[,'traitlog_cumdt:lcl_max'])-1
summary(lcl_effect)
```
...
st_re <- which( grepl(colnames(chains2$VCV), pattern = "traitlog_propagule")) # var comp for ini size
...
tax_eff_st <- rbind(
  quantile(chains2$VCV[,"traitlog_propagule.parasite_genus"]/
             rowSums(chains2$VCV[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_propagule.parasite_family"]/
             rowSums(chains2$VCV[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_propagule.parasite_order"]/
             rowSums(chains2$VCV[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_propagule.parasite_class"]/
             rowSums(chains2$VCV[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_propagule.parasite_phylum"]/
             rowSums(chains2$VCV[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(rowSums(chains2$VCV[,st_re[1:5]])/
             rowSums(chains2$VCV[,st_re]), probs = c(0.025, 0.5, 0.975))
)
...
tax_eff_fs <- rbind(
  ...
  quantile(chains2$VCV[,"traitlog_end.parasite_phylum"]/
             rowSums(chains2$VCV[,fs_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(rowSums(chains2$VCV[,fs_re[1:5]])/
             rowSums(chains2$VCV[,fs_re]), probs = c(0.025, 0.5, 0.975))
)
...
# devo time
resi <- which( grepl(colnames(chains2$VCV), pattern = ".units")) # res var-cov
dt_re <- which( grepl(colnames(chains2$VCV), pattern = "traitlog_cumdt")) # var comp for dt
...
tax_eff_dt <- rbind(
  quantile(chains2$VCV[,"traitlog_cumdt.parasite_genus"]/
             rowSums(chains2$VCV[,dt_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_cumdt.parasite_family"]/
             rowSums(chains2$VCV[,dt_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_cumdt.parasite_order"]/
             rowSums(chains2$VCV[,dt_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_cumdt.parasite_class"]/
             rowSums(chains2$VCV[,dt_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(chains2$VCV[,"traitlog_cumdt.parasite_phylum"]/
             rowSums(chains2$VCV[,dt_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(rowSums(chains2$VCV[,dt_re[1:5]])/
             rowSums(chains2$VCV[,dt_re]), probs = c(0.025, 0.5, 0.975))
)
...
```{r}
f5b <- ggplot(dxy, aes(x = lcl_max_fac, y = biovolume)) +
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) +
  annotate('text', label = "(b)", x = 0.8, y = max(dx$biovolume, na.rm = T))
f5b
```
...
```{r}
ggplot(dxy, aes(x = lcl_max_fac, y = propagule)) +
  geom_boxplot(outlier.colour = NA) +
  geom_point(alpha = 0.5, size = 0.5,
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) +
  annotate('text', label = "(x)", x = 0.8, y = max(dxy$propagule, na.rm = T))
```
...
```{r}
f5a <- ggplot(dxy, aes(x = lcl_max_fac, y = cum_dt/15)) +
  geom_boxplot(outlier.colour = NA) +
  geom_point(alpha = 0.5, size = 0.5,
             aes(shape = is.na(cum_dt), color = lcl_max_fac),
             position = position_jitter(width = 0.33, height = 0)) +
  scale_y_log10() +
  scale_x_discrete(labels = c("1", "2", "3", "4 or 5")) +
  scale_shape_manual(values = c(19,4)) +
  ...
  guides(shape = F, color = F) +
  labs(x = "Life cycle length", 
       y = "Age at maturity (days at 20°C)") +
  geom_pointrange(data = p_all, 
                  aes(x = lcl_max, y = exp(cum_dt.fit)/15,
                      ymin = exp(cum_dt.lwr)/15, ymax = exp(cum_dt.upr)/15),
                  size = 1.) +
  geom_line(data = p_all,
            aes(group = group, x = lcl_max, y = exp(cum_dt.fit)/15),
            size = 1.25, linetype = "dashed") +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) +
  annotate('text', label = "(a)", x = 0.8, y = 750) +
  coord_cartesian(ylim = c(min(dxy$cum_dt/15, na.rm = T), 750) )
f5a
```
...
mutate(p_all, prev_ws_avg = lag(cum_dt.fit))%>%
  ...
  mutate(fold_change = untransformed_mass/nh, 
         perc_increase = ((untransformed_mass-nh)/nh) * 100)%>%
  mutate(cum_dt.lwr = round(exp(cum_dt.lwr)/15,1), 
         cum_dt.fit = round(exp(cum_dt.fit)/15,1), 
         cum_dt.upr = round(exp(cum_dt.upr)/15,1))%>%
...
```{r}
dxy <- mutate(dxy, rgr = rg/(cum_dt/15))
f5c <- ggplot(dxy, aes(x = lcl_max_fac, y = rgr)) +
  geom_boxplot(outlier.colour = NA) +
  geom_point(alpha = 0.5, size = 0.5,
             aes(shape = is.na(biovolume)|is.na(propagule)|is.na(cum_dt),
                 color = lcl_max_fac),
             position = position_jitter(width = 0.33, height = 0)) +
  scale_x_discrete(labels = c("1", "2", "3", "4 or 5")) +
  scale_shape_manual(values = c(19,4)) +
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.x = element_blank()) +
  annotate('text', label = "(c)", x = 0.8, y = 0.55) +
             # max(dxy$rgr, na.rm = T)) +
...
```{r}
f5 <- cowplot::plot_grid(f5a, f5b, f5c, nrow = 1, align = 'hv')
```
```{r}
title <- ggdraw() +
...
```
```{r}
ggsave(f5, filename = "../../figs/fig5_noimpDD.svg", device = 'svg', width = 8, height = 3)
ggsave(f5, filename = "../../figs/fig5_noimpDD.png", device = 'png', width = 8, height = 3)
ggsave(f5gif2, filename = "../../figs/gif_dd_vs_nodd/fig5_noimpdd.png",
       width = 8, height = 3)
rm(f5gif2)
```
...
x <- posterior.mode(posterior.cor(chains2$VCV[,16:24]))
...
```{r}
HPDinterval((posterior.cor(chains2$VCV[,16:24])))
```
...
```{r}
save.image(file = "after_splevel_noimp_modDD.RData")
```
