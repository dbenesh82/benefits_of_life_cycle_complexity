...
incomp_spp <- dat%>%
  filter(is.na(avg_dd), is_paratenic != 1, Facultative != 'postcyclic')%>%
...
And this trend might partly be due to imputed values.
...
Now we look at age at maturity - the total time spent developing across all hosts. We plot this as "days at room temp".

```{r}
f5a <- ggplot(dxy, aes(x = lcl_max_fac, y = cum_dt/15)) +
  geom_boxplot(outlier.colour = NA) +
  geom_point(alpha = 0.5, size = 0.5,
             aes(shape = is.na(cum_dt_ni), color = lcl_max_fac),
             position = position_jitter(width = 0.33, height = 0)) +
  scale_y_log10() +
  scale_x_discrete(labels = c("1", "2", "3", "4 or 5")) +
  scale_shape_manual(values = c(19,4)) +
...
```
```{r}
ggsave(f5, filename = "../../figs/fig5_dd_imp.svg", device = 'svg', width = 8, height = 3)
ggsave(f5, filename = "../../figs/fig5_dd_imp.png", device = 'png', width = 8, height = 3)
ggsave(f5gif2, filename = "../../figs/gif_dd_vs_nodd/fig5_impdd.png",
       width = 8, height = 3)
rm(f5gif2)
```
...
```{r}
save.image(file = "after_splevel_multiv_modDD.RData")
```
