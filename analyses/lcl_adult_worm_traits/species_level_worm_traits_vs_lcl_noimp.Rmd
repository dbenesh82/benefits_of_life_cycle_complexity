...
dat <- mutate(dat, avg_dt_p = if_else(is_paratenic == 1, 3, avg_dt))
...
incomp_spp <- dat%>%
  filter(is.na(avg_dt_p), is_paratenic != 1, Facultative != 'postcyclic')%>%
  .$Parasite.species
...
```{r}
ggplot(dx, aes(x = lcl_max_fac, y = biovolume)) +
...
```{r}
ggplot(dx, aes(x = lcl_max_fac, y = propagule)) +
  geom_boxplot(outlier.color = NA) +
  geom_point(alpha = 0.2, position = position_jitter(width = 0.3)) +
  scale_y_log10() +
...
What about propagule size? Do parasites with different life cycle lengths enter the environment at different sizes? No, propagule size did not improve the model.
...
```
```{r}
ggsave(f5, filename = "../../figs/fig5_noimp.svg", device = 'svg', width = 8, height = 3)
ggsave(f5, filename = "../../figs/fig5_noimp.png", device = 'png', width = 8, height = 3)

ggsave(f5gif, filename = "../../figs/gif_imp_vs_noimp/fig5_noimp.png",
       width = 8, height = 3)
ggsave(f5gif2, filename = "../../figs/gif_dd_vs_nodd/fig5_noimp.png",
       width = 8, height = 3)
rm(f5gif, f5gif2)
```
...
```{r}
save.image(file = "after_splevel_noimp_mod.RData")
```
