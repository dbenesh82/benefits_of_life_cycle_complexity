...
for(i in 1:100){
  ...
  # first filter to just data including fixed predictors
  dxyz_imp <- filter(dat, !(is.na(imp_avg_dt) | is.na(imp_biovolume) | is.na(imp_initial_biov)))
...
tl_asy <- nls(log(imp_biovolume) ~ Asym+(R0-Asym)*exp(-exp(lrc)*cum_dt),
          start = iPar,
          data = cum_dd_sp)
# summary(tl_asy)
...
x<-0:150
lg2 <- data.frame(x = x, y = exp( predict(tl_asy_ni, newdata = data.frame(cum_dt_ni = x)) )
