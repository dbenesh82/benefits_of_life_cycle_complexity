...
dat <- mutate(dat, Host_no_fac = factor(Host_no_fac),
  ...
  mutate(log_end = log(imp_biovolume), log_start = log(imp_initial_biov), 
         log_dt = log(imp_avg_dd))%>%
  mutate(rg = log_end - log_start, rgr = (log_end - log_start)/imp_avg_dd)%>%
  mutate(rgr_doubles = (log(imp_biovolume, base = 2) - log(imp_initial_biov, base = 2))/imp_avg_dd)%>%
...
dxy <- filter(dat, !(is.na(imp_avg_dd) | is.na(imp_biovolume) | is.na(imp_initial_biov)))
...
```{r}
f2e <- ggplot(dxy,
              aes(x = exp(host_bm), y = imp_avg_dd, color = endo_ecto)) + 
  geom_point(alpha = 0.4, size = 0.65,
  ...
  annotate('text', label = "(e)", 
           x = min(exp(dxy$host_bm), na.rm = T),
           y = max(dxy$imp_avg_dd, na.rm = T))
f2e
```

```{r}
f2f <- ggplot(dxy,
              aes(x = host_tl, y = imp_avg_dd, color = endo_ecto)) + 
  geom_point(alpha = 0.4, size = 0.65,
             aes(shape = is.na(avg_dd))
             ) + 
...
```{r}
# 10 g host
filter(p_all, host_tl == 2.7, trait == "log_dt_p")%>%
  mutate(diff_from_wanted = abs(host_bm - log(10)))%>%
  arrange(diff_from_wanted)%>%
  group_by(endo_ecto)%>%slice(1)%>%
...
```{r}
# 10 g host
filter(p_all, host_tl == 2.7, trait == "log_dt_overaweek")%>%
  mutate(diff_from_wanted = abs(host_bm - log(10)))%>%
  arrange(diff_from_wanted)%>%
  group_by(endo_ecto)%>%slice(1)%>%
  select(dt_10g = fit, dt_10g_lwr = lwr, dt_10g_upr = upr)%>%
...
```{r}
dxy <- mutate(dxy, rgr = (log_end-log_start)/(imp_avg_dd/15)) # adjust rgr so it is per day at 20 C
f2g <- ggplot(filter(dxy, imp_avg_dd>90),
              aes(x = exp(host_bm), y = (log_end-log_start)/(imp_avg_dd/15), color = endo_ecto)) + 
  geom_point(alpha = 0.4, size = 0.65,
  ...
  annotate('text', label = "(g)", 
           x = min(exp(dxy$host_bm), na.rm = T),
           y = max(filter(dxy, imp_avg_dd>90)$rgr, na.rm = T))
f2g
```
...
```{r}
f2h <- ggplot(filter(dxy, imp_avg_dd>90),
              aes(x = host_tl, y = (log_end-log_start)/(imp_avg_dd/15), color = endo_ecto)) + 
  geom_point(alpha = 0.4, size = 0.65,
             aes(shape = is.na(initial_biov)|is.na(biovolume)|is.na(avg_dd))
             ) + 
...
```{r}
title <- ggdraw() +
  draw_label("Imputed, degree days",
    fontface = 'bold',
    size = 20
  ) 
...
```{r}
ggsave(f2, filename = "../../figs/fig3_dd_imp.png", width = 8, height = 6)
ggsave(f2, filename = "../../figs/fig3_dd_imp.svg", width = 8, height = 6)
# move plots closer together, fix legend pos
ggsave(f2dd, filename = "../../figs/gif_dd_vs_nodd/fig3_impdd.png", width = 8, height = 6)
rm(f2dd)
```
...
```{r}
# first filter to just data including fixed predictors
dxy_dh <- filter(dat, !(is.na(imp_avg_dd) & is.na(imp_biovolume) & is.na(imp_initial_biov)))
...
Growth *rates* are not faster in endotherm definitive hosts when we control for temperature.
...
f2c_rg_dh
```
However, although they growth more in endotherm definitive hosts, the average prepatent period is not much longer.

```{r}
f2e_dh <- ggplot(filter(dxy, imp_avg_dd>90),
              aes(x = exp(host_bm), y = imp_avg_dd, color = endo_ecto,
                  linetype = Def.int)) + 
  geom_point(alpha = 0.4, size = 0.65,
             aes(shape = is.na(avg_dd))
...
f2e_dh
```
In endothermic definitive hosts, worms grow more with a slightly longer developmental period, so growth rate does not differ between endo and ectothermic definitive hosts.

```{r}
f2g_dh <- ggplot(filter(dxy, imp_avg_dd>90),
              aes(x = exp(host_bm), y = (log_end-log_start)/imp_avg_dd, 
...
Stage explains additional variation in devo time (different from unimputed analysis, due to paratenics).
...
```{r}
# first filter to just data including fixed predictors
dxyz <- filter(dat, !(is.na(imp_avg_dd) | is.na(imp_biovolume) | is.na(imp_initial_biov)))
...
incomp_spp <- dxyz%>%
  filter(is.na(avg_dd), is_paratenic != 1)%>%
...
dxyz <- dxyz%>%
  group_by(Parasite.species)%>%
  mutate(cum_dt = if_else(!Parasite.species %in% nspp, cumsum(imp_avg_dd), NA_real_),
         cum_dt_ni = if_else(!Parasite.species %in% incomp_spp, cumsum(imp_avg_dd), NA_real_))%>%
...
# filter(dxy, pred == "no")%>%
#   select(Parasite.species, Host_no_fac, lcl_max_fac, avg_dd, imp_avg_dd)%>%
#   mutate(removed = Parasite.species %in% nspp)%>%
#   group_by(Parasite.species)%>%
#   mutate(cum_dt = cumsum(imp_avg_dd))%>%
...
for(i in 1:100){
  ...
  # first filter to just data including fixed predictors
  dxyz_imp <- filter(dat, !(is.na(imp_avg_dd) | is.na(imp_biovolume) | is.na(imp_initial_biov)))
...
Prepatent period, in degree days, decreases with life cycle length.

```{r}
sx <- s[c("traitlog_dt", "traitlog_dt:stage_lcllc2_2", "traitlog_dt:stage_lcllc3_3", 
          "traitlog_dt:stage_lcllc3+_4"), "quant.50."]
sx <- data.frame(lcl = 1:4,
                 prepatent = c(sx[1],
                                         sx[1]+sx[2],
                                         sx[1]+sx[3],
                                         sx[1]+sx[4]))
sx <- mutate(sx, prev_ws_avg = lag(prepatent))%>%
  mutate(ord_mag_change = prepatent - prev_ws_avg)%>%
  mutate(days_rt = exp(prepatent)/15)%>%
  mutate(nh = lag(days_rt))%>%
  mutate(fold_change = days_rt/nh, 
         perc_decrease = (abs(days_rt-nh)/nh) * 100)
sx <- select(sx, lcl, prepatent, days_rt, ord_mag_change, fold_change, perc_decrease)
sx
rm(sx)
```
...
```
```{r}
f3c <- ggplot(filter(dxy, !is.na(imp_avg_dd)),
       aes(x = Host_no_fac, y = imp_avg_dd)) +
  geom_boxplot(outlier.color = NA) +
  geom_point(alpha = 0.33, size = 0.75,
             aes(shape = is.na(avg_dd)),
             position = position_jitter(width = 0.33, height = 0)) +
...
```{r}
ggplot(filter(dxy, !is.na(imp_avg_dd), Stage == "adult"),
       aes(x = Host_no_fac, y = imp_avg_dd)) +
  geom_boxplot(outlier.color = NA) +
  geom_point(alpha = 0.33, size = 0.75,
             aes(shape = is.na(avg_dd)),
...
```
```{r}
f3d <- ggplot(filter(dxy, !is.na(imp_biovolume), !is.na(imp_initial_biov), !is.na(imp_avg_dd), imp_avg_dd>90),
       aes(x = Host_no_fac, y = (log_end - log_start)/(imp_avg_dd/15))) +
  geom_boxplot(outlier.color = NA) +
  geom_point(alpha = 0.33, size = 0.75,
             aes(shape = is.na(biovolume)|is.na(initial_biov)|is.na(avg_dd)),
             position = position_jitter(width = 0.33, height = 0)) +
  ...
  geom_point(data = filter(p_all3_wide, sig == "sig")%>%
               mutate(y = -0.2), #min( ((dxy$log_end - dxy$log_start)/dxy$imp_avg_dd), na.rm = T)),
             aes(x = Host_no_fac, y = y), 
             shape = 23, size = 3, stroke = 3,
             fill = RColorBrewer::brewer.pal(3, "Pastel1")[1],
             color = RColorBrewer::brewer.pal(3, "Pastel1")[2]) +
...
```{r}
ggplot(filter(dxy, !is.na(imp_biovolume), !is.na(imp_initial_biov), !is.na(imp_avg_dd), imp_avg_dd>90, Stage == "adult"),
       aes(x = lcl_max_fac, y = (log_end - log_start)/(imp_avg_dd/15))) +
  geom_boxplot(outlier.color = NA) +
  geom_point(alpha = 0.33, size = 0.75,
             aes(shape = is.na(biovolume)|is.na(initial_biov)|is.na(avg_dd)),
             position = position_jitter(width = 0.33, height = 0)) +
...
```
```{r}
ggsave(f3, filename = "../../figs/fig4_dd_imp.png",
       width = 8, height =  3.5*2)
ggsave(f3, filename = "../../figs/fig4_dd_imp.svg",
       width = 8, height = 3.5*2)
# move closer together in inkscape, adjust labels
ggsave(f3dd, filename = "../../figs/gif_dd_vs_nodd/fig4_impdd.png",
...
cum_dd <- ungroup(cum_dd_sp)%>%
  ...
  summarise(log_start_p.fit = median(log_start, na.rm=T),
            ...
            log_dt_ni.fit = median(log(avg_dd), na.rm=T),
...
Here is the median cumulative development time (days at rt) for direct vs complex life cycles...
...
cum_dd_sp%>%
  ...
  summarize(n_imp = sum(!is.na(cum_dt)),
            ...
            med_cum_dt_ni = median(cum_dt_ni/15, na.rm = T))
```
...
ggplot(filter(cum_dd_sp, ), 
       aes(x = cum_dt/15, y = imp_biovolume, color = lcl_max_fac)) +
  scale_shape_manual(values = c(19,4)) +
...
iPar <- getInitial(log(imp_biovolume) ~ SSasymp(cum_dt_rt, Asym, R0, lrc), data = cum_dd_sp)
tl_asy2 <- nls(log(imp_biovolume) ~ Asym+(R0-Asym)*exp(-exp(lrc)*cum_dt_rt),
          start = iPar,
          data = cum_dd_sp)
# summary(tl_asy2)
# summary(tl_asy) # exactly the same, just different parameterizations

iPar <- getInitial(log(imp_biovolume) ~ SSweibull(cum_dt_rt, Asym, Drop, lrc, pwr), data = cum_dd_sp)
tl_w <- nls(log(imp_biovolume) ~ Asym-Drop*exp(-exp(lrc)*cum_dt_rt^pwr),
          start = iPar,
...
```{r}
tl_lm <- lm(log(imp_biovolume) ~ cum_dt_rt, data = filter(cum_dd_sp,))
data.frame(line_res_se = summary(tl_lm)$sigma, curve_res_se = summary(tl_asy)$sigma)
```
Here are the curves parameters:
...
f5 <- ggplot(filter(cum_dd_sp, ), 
  ...
  theme(panel.grid.minor = element_blank()) 
f5
ggsave(f5, filename = "../../figs/fig5d_dd_imp.png", width = 8, height = 3)
ggsave(f5, filename = "../../figs/fig5d_dd_imp.svg", width = 8, height = 3)
```
...
```{r}
ggplot(filter(cum_dd_sp, ), 
       aes(x = cum_dt_rt, y = imp_biovolume, color = lcl_max_fac)) +
  scale_shape_manual(values = c(19,4)) +
...
```{r}
cum_dd_sp <- mutate(cum_dd_sp, cum_dt_ni_rt = cum_dt_ni/15)
iPar <- getInitial(log(imp_biovolume) ~ SSasymp(cum_dt_ni_rt, Asym, R0, lrc), 
                   data = filter(cum_dd_sp, !is.na(cum_dt_ni_rt)))
tl_asy_ni <- nls(log(imp_biovolume) ~ Asym+(R0-Asym)*exp(-exp(lrc)*cum_dt_ni_rt),
          start = iPar,
          data = filter(cum_dd_sp, !is.na(cum_dt_ni_rt)))
# summary(tl_asy_ni)
...
x<-0:250
lg2 <- data.frame(x = x, y = exp( predict(tl_asy_ni, newdata = data.frame(cum_dt_ni_rt = x)) ))
```
```{r}
ggplot(filter(cum_dd_sp, ), 
       aes(x = cum_dt_ni_rt, y = biovolume, color = lcl_max_fac)) +
  scale_shape_manual(values = c(19,4)) +
  # scale_x_log10() +
  ...
  geom_line(data = lg2, aes(x = x, y = y),
              linetype = "dashed", color = "black", size = 1.5) +
  geom_line(data = filter(cum_dd, model == "observed"), 
            aes(x = (exp(cum_dt_ni.fit)-1)/15, y = exp(log_end_ni.fit), group = lcl_max_fac),
            size = 1.5) +
  # geom_linerange(data = filter(cum_dd, model == "observed"), 
  ...
  #           alpha = 1, ) +
  geom_label(data = filter(cum_dd, model == "observed"),
             aes(x = (exp(cum_dt_ni.fit)-1)/15, y = exp(log_end_ni.fit), label = Host_no_fac), 
             fill = "white",
             size = 4) +
  labs(x = 'Cumulative time developing (days at 20°C)', y = 'Final worm size', color = 'LCL') 
```
...
```{r}
save.image(file = "after_multi_modelsDD.RData")
```
